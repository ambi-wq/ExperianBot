    ...
    try:
        ...
        if operation_id != None:
            ...
            query_data = create_query(
                "select response,HINT_TEXT from operations_MyTaj where op_id = '" + str(operation_id) + "' ")
        ...
        if Sub_Benefits_id != None:
            ...
            query_data = create_query(
                "select op_id,HINT_TEXT,Sub_Benefits_Type from operations_MyTaj where Sub_Benefits_id = '" + Sub_Benefits_id + "' AND Location_id ='" + location_id + "' ")
            list_data = list(query_data)
            sub_benefits = ""
                ...
                for t_list in tag_list:
                    ...
                    if len(t_list) == 16:
                        ...
                        if "|" in res_data:
                            ...
                            data1 = json.dumps(json_data, ensure_ascii=False)
                            print("json data = ", data)
                            dbInsertion(temtext, data1, "answered", username, userType, userID, "-1", mainIssue, "HR",
                                        "-1")
                            resp = flask.Response(encrypt(data))
