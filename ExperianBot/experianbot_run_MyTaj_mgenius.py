...
os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = "D:\SuperEChatBot-0b3d157f68ce.json"
...
## add mgenius path
BasePathAttachment = "https://mgenius.in/SuperEBotAdmin/static/notifications_attachments/"
BasePathDocumnets = "https://mgenius.in/SuperEBotAdmin/static/document_files/"
...
def Notifications():
    if request.method == "POST":
        ...
        print(result)
        currentdate = datetime.now().strftime("%Y-%m-%d")
        leave_query = "INSERT INTO notify_MyTaj( Title, Description, Sender, date) " \
...
def set_reminder_def():
    try:
        ...
        if remGoogleFlag == "true" or remGoogleFlag == 'True':
            ...
            email_query = "select EmailId from Employees where EmpId = '" + str(userID) + "'"
        ...
        if len(tags_list) > 0:
            ...
            formdata = "UPDATE chat_history SET query = '" + data2 + "' where asked_on < '" + currenttime + "' order by id desc "
...
def applyleavedef():
    try:
        ...
        his_data = "SELECT leave_detail.emp_id,leave_detail.type,leave_detail.from_date,leave_detail.to_date," \
                   ...
                   "WHERE leave_detail.insert_time > chat_history.asked_on order by chat_history.id desc "
        output = create_query(his_data)
        tags_list = list(output)
        json_array = []
        ...
        if len(tags_list) > 0:
            ...
            data2 = json.dumps(json_data)
            print("json data ==> ", data)
            leaveform = "UPDATE chat_history SET query = '" + data2 + "' where asked_on < '" + currenttime + "' order by id desc "
...
def get_chat_history():
    ...
    query = "select TOP 10 id,asked_on,query,flag from chat_history where (cast(asked_on as date) between '" + str(
        predate) + "'and '" + str(currentdate) + "') and userID = '" + str(userID) + "' order by id desc"
    print(query)
    output = create_query(query)
...
def gettext(text,language):
    ...
    elif text == "No Tickets are found":
        a = {
            "gu": "કોઈ ટિકિટ મળી નથી",
            "hi": "कोई टिकट नहीं मिला",
            "ml": "ടിക്കറ്റുകളൊന്നും കണ്ടെത്തിയില്ല",
            "mr": "कोणतीही तिकिटे आढळली नाहीत",
            "ta": "டிக்கெட் எதுவும் கிடைக்கவில்லை",
            "te": "టికెట్లు కనుగొనబడలేదు",
            "ur": "کوئی ٹکٹ نہیں ملا",
            "pa": "ਕੋਈ ਟਿਕਟ ਨਹੀਂ ਮਿਲੀ",
        }
...
def get_bot_response():
    ...
    Sub_Benefits_id = user_text.get('Sub_Benefits_id')
    ...
    if locationId != None or locationId != "":
        getLocation = create_query("select location_name from taj_location where location_id = '"+str(locationId)+"'  ")
        Location = getLocation[0][0]
        print("Location_Name",Location)


    print("Sub_Benefits_id", Sub_Benefits_id)

    if Sub_Benefits_id == "":
        Sub_Benefits_id = None
    ...
    tic_status = "Ticket Status"
    if language_flag:
        tic_status = gettext(tic_status, language)

    tic_status_found = "No Tickets are found"
    if language_flag:
        tic_status_found = gettext(tic_status_found, language)
    ...
    try:
        ...
        if main_op_id != None:
            ...
            json_array = []
            bot_resp = ""
            query_data = create_query("select * from main_operations_MyTaj where main_op_id = '" + main_op_id + "' ")
            list_data = list(query_data)
            print("list_data----------------",list_data)
            print("usertext--------------",temtext)
            for ld in list_data:
                # print("ld================ ", language_flag)
                if language_flag:
                    if language == 'gu':
                        bot_resp = ld[11]
                    if language == 'hi':
                ...
                headin_opid = "<b>"+ str(temtext) +"</b>"
                # if language_flag:
                #     headin_opid = translate_text(ld[10], language)
                # else:
                #     headin_opid = ld[10]
                ...
                if "|" in bot_resp:
                    ...
                    for sp in sp_data:
                        if "$" in sp:
                            ...
                            json_array.append(sim)
                            json_data = {
                                "message": wldhappy +" "+ str(headin_opid) + " ",
                                "action": json_array,
                                "typetext": "HR",
                    ...
                    print("json data = ", data)
                    dbInsertion(temtext, data1, "answered", username, userType, userID, "-1", mainIssue,"HR", "-1",Location)
                    resp = flask.Response(encrypt(data))
                    # resp = flask.Response(data)
        ...
        if operation_id != None:
            ...
            query_data = create_query(
                "select response,HINT_TEXT,gu,hi,ml,mr,pa,ta,te,ur from operations_MyTaj where op_id = '" + str(operation_id) + "' ")
            # query_data = create_query("select response,heading from operations1 where operation_id = '" + str(operation_id) + "' ")
            print("res == ", query_data)
            bot_response = query_data[0][0]
            print(bot_response)
            ...
            #     bot_resp = translate_text(bot_resp, language)

            if language_flag:
                if language == 'gu':
                    bot_resp = query_data[0][2]
                if language == 'hi':
                    bot_resp = query_data[0][3]
                if language == 'ml':
                    bot_resp = query_data[0][4]
                if language == 'mr':
                    bot_resp = query_data[0][5]
                if language == 'pa':
                    bot_resp = query_data[0][6]
                if language == 'ta':
                    bot_resp = query_data[0][7]
                if language == 'te':
                    bot_resp = query_data[0][8]
                if language == 'ur':
                    bot_resp = query_data[0][9]
            else:
                bot_resp = bot_response

            print("bot_resp------",bot_resp)
            ...
            data = json.dumps(json_data)
            print("json data = ", data)
            dbInsertion(temtext, data, "answered", username, userType, userID, "-1", mainIssue, "HR", "-1",Location)
            resp = flask.Response(encrypt(data))
            # resp = flask.Response(data)
            logger.info(f'{userID} [get] Response:' + str(json_data))
            return resp

        if Sub_Benefits_id != None:
            print("in Sub_Benefits_id")
            json_array = []
            location_id = locationId.upper()
            query_data = create_query(
                "select op_id,HINT_TEXT,Sub_Benefits_Type,hint_hi,hint_gu,hint_ml,hint_mr,hint_pa,hint_ta,hint_te,hint_ur from operations_MyTaj where Sub_Benefits_id = '" + Sub_Benefits_id + "' AND Location_id ='" + location_id + "' ")
            list_data = list(query_data)
            sub_benefits = ""
        ...
        if '**' in bot_data:
            ...
            try:
                ...
                print(e)
                dbInsertion(temtext, data2, "query", username, userType, userID, "-1", mainIssue, "--", "-1",Location)
            ...
            for opData in output2:
                # print(opData[7])
                locvalues = opData[7]
                valueLocList = locvalues.lower().split(',')
                print(valueLocList)
                if locationId.lower() in valueLocList:
            ...
            tag_list = resultList
            tag_list.sort(key=myDocFunc, reverse=True)
            print(tag_list)
            ...
            dbInsertion(temtext, data2, "answered", username, userType, userID, "-1", mainIssue, "--", "-1",Location)
            print(data)
            # resp = flask.Response(data)
            resp = flask.Response(encrypt(data))
            return resp
            ...
            resultList = []
            query = "SELECT * FROM notify_MyTaj WHERE  value='All' and Status='Active' "
            output = create_query(query)
            print('notify 1-->', output)
            for op in output:
                resultList.append(op)

            query1 = "SELECT * FROM notify_MyTaj WHERE flag = 'Individual' and Status='Active'"
            output1 = create_query(query1)
            ...
            query2 = "SELECT * FROM notify_MyTaj WHERE flag = 'Location' and Status='Active'"
            output2 = create_query(query2)
        ...
        if flag:
            ...
            if bot_data == "ERROR":
                print("if error")
                json_data = {
                    "error": "I am sorry, we could not find a solution for your reported concern. Would you like to send a mail to Help Desk Team?",
                ...
                for t_list in tag_list:
                    ...
                    if len(t_list) == 13:
                        ...
                        print(locationId.upper())
                        if locationId.upper() == Location:
                            op_id = t_list[0]
                            Sub_Benefits_Type = t_list[3]
                    ...
                    if len(t_list) == 16:
                        ...
                        headin = "<b>"+ str(temtext) +"</b>"
                        print(headin)
                        # if language_flag:
                        #     headin = translate_text(t_list[7], language)
                        # else:
                        #     headin = t_list[7]

                        print("language_flag0",language_flag)
                        print("Language",language)
                        if language_flag:
                            if language == 'gu':
                                res_data = t_list[8]
                        ...
                        if "|" in res_data:
                            ...
                            data1 = json.dumps(json_data, ensure_ascii=False)
                            print("json data = ", data)
                            dbInsertion(temtext, data1, "answered", username, userType, userID, "-1", mainIssue,
                                        "HR", "-1",Location)
                            resp = flask.Response(encrypt(data))
                            # resp = flask.Response(data)
                            logger.info(f'{userID} [get] Response:' + str(json_data))
                    ...
                    if len(t_list) == 14:
                        ...
                        location_id_res = t_list[5]

                        if locationId.upper() == location_id_res:

                            query_response = str(t_list[2])

                            if language_flag:
                                if language == 'gu':
                                    query_response = t_list[6]
                                if language == 'hi':
                                    query_response = t_list[7]
                                if language == 'ml':
                                    query_response = t_list[8]
                                if language == 'mr':
                                    query_response = t_list[9]
                                if language == 'pa':
                                    query_response = t_list[10]
                                if language == 'ta':
                                    query_response = t_list[11]
                                if language == 'te':
                                    query_response = t_list[12]
                                if language == 'ur':
                                    query_response = t_list[13]
                            else:
                                query_response = str(t_list[2])
                ...
                if lastFlag:
                    ...
                    json_data = {
                        "message": wldhappy +" " + str(temtext) + "</b>",
                        "ParameterTitle": "ListView",
                        "typetext": typetext,
                        "action": json_array,
                        "mainIssue": mainIssue
                ...
                data = data1
                data2 = json.dumps(json_data)
                dbInsertion(temtext, data2, "answered", username, userType, userID, "-1", mainIssue, category, "-1",Location)
                resp = flask.Response(encrypt(data))
                # resp = flask.Response(data)
                logger.info(f'{userID} [get] Response:' + str(json_data))
                return resp
                ...
                print("json data == ", data1)
                data = json.dumps(json_data)
                dbInsertion(temtext, data, "query", username, userType, userID, "-1", mainIssue, "--", "-1",Location)
                resp = flask.Response(encrypt(data))
                # resp = flask.Response(data)
            ...
            if bot_confidence > 0.50 or fuzzymatch > 60:
                ...
                if equalMatch:
                    ...
                    data = data1
                    data2 = json.dumps(json_data)
                    dbInsertion(temtext, data2, "unanswered", username, userType, userID, "-1", mainIssue, "--", "-1",Location)
                    resp = flask.Response(encrypt(data))
                    # resp = flask.Response(data)
                    resp.headers['Access-Control-Allow-Origin'] = '*'
                    logger.info(f'{userID} [get] Response:' + str(json_data))
                    ...
                    data1 = json.dumps(json_data, sort_keys=True, indent=4 * ' ')
                    print("json data = ", data1)
                    dbInsertion(temtext, "I am sorry, we could not find a solution for your reported concern. "
                                         "Would you like to raise a ticket to the Help Desk Team?", "unanswered",
                                username, userType,
                                userID,
                                "-1", mainIssue, "--", "-1",Location)
                    data = json.dumps(json_data)
                    resp = flask.Response(encrypt(data))
                    # resp = flask.Response(data)
        ...
        json_data = {
            "error": "We are sorry that we could not resolve your concern. Would you like to send a mail to the Help Desk Team?",
            "resolve": "",
            "rating": "no", "mainIssue": mainIssue
        }
...
def dbInsertion(usertext, response, flag, username, userType, userID, feedback, mainIssue, category, emailSent,Location):
    print("db fun" + "-" * 50)
    print(usertext, response, flag, username, userType, userID, feedback, mainIssue, category, emailSent,Location)
    try:
        print("db flag==", flag)
        ...
        if flag.lower() == "unanswered":
            unanswered_query = "insert into unanswered_queries" \
                               "(query,asked_on,username,userType,userid,category,location)" \
                               "values(N'" + str(usertext) + "','" + currenttime + "','" + str(username) + "','" + str(
                userType) + "','" + str(
                userID) + "','" + str(mainIssue) + "','" + str(Location) + "')"
            ...
            inserted_data = create_query("SELECT * FROM unanswered_queries where username = '" + str(
                username) + "' order by asked_on desc")
            print("####@@", inserted_data)
            ...
            answered_query = "insert into answered_queries" \
                             "(query,asked_on,username,userType,query_answer,userID,feedback,category,email_sent,location)" \
                             "values(N'" + str(usertext) + "','" + currenttime + "','" + str(username) + "','" + str(
                userType) + "','" + str(
                response) + "','" + str(userID) + "','" + str(feedback) + "','" + str(category) + "','" + str(
                emailSent) + "','" + str(Location) + "')"
...
def get_bot_response1111():
    try:
        ...
        if userResponse.lower() == 'no':  # if users issue did not resolved
            try:
                getAnswered_query = "SELECT query_id FROM answered_queries where username='" + userName + "' and Feedback='-1' order by asked_on desc"
                ...
                # print("Exception Occured " + e)
            resp = flask.Response(encrypt(
                "We are sorry that we could not resolve your concern. Would you like to raise a ticket to the Help Desk Team?"))
            resp.headers['Access-Control-Allow-Origin'] = '*'
        ...
        if userResponse.lower() == 'yes':  # if users issue resolved
            ...
            if response.get("code"):
                ...
                resp = flask.Response(encrypt(
                    "Thank you, We have created Ticket for your concern.<br><b><center>Ticket ID&nbsp;<font size='5px' color='#174c82'>" + Ticket_id_ + "</font></center>"))
...
def send_email():
    ...
    if userResponse == "yes":
        ...
        #   mail.send(msg)
        get_email_query = "SELECT query_id from answered_queries WHERE username = '" + username + "' AND tabFeedBack = '--' ORDER BY asked_on desc  "
...
def get_feedback_rating():
    ...
    try:
        if str(UserRespValue).lower() == "yes":
            get_ans_query = "SELECT query_id FROM answered_queries WHERE username = '" + username + "' and userID = '" + userID + "' order by asked_on desc"
            ...
            get_ans_query = "SELECT query_id FROM answered_queries WHERE username = '" + username + "' and userID = '" + userID + "' and feedback = 'no' order by asked_on desc "
            ans = create_query(get_ans_query)
            query_id = ans[0][0]
            if len(ans) > 0:
                answered_query = "UPDATE answered_queries SET rating ='" + rating + "' , rating_feedback ='" + RatingDesIssue + "' WHERE query_id = '" + str(
            ...
            get_ans_query = "SELECT query_id FROM unanswered_queries WHERE username = '" + username + "' and userID = '" + userID + "' order by asked_on desc "
            ans = create_query(get_ans_query)
            query_id = ans[0][0]
            if len(ans) > 0:
                answered_query = "UPDATE answered_queries SET rating ='" + rating + "' , rating_feedback ='" + RatingDesIssue + "' WHERE query_id = '" + str(
...
def get_recommendation_list():
    ...
    query_data = create_query("SELECT DISTINCT CONVERT(VARCHAR(MAX), Question) FROM operations_MyTaj")
...
def get_issue_ticket():
    try:
        filestoragearray = []
        # posted_file = request.files['descAttachmentOne']
        # print(posted_file)
        ...
        if userResponse.lower() == 'yes':
            ...
            if response.get("code"):
                ...
                if (response['requestNumber'] == "Ticket Creation failed!!"):
                    ...
                    resp.headers['Access-Control-Allow-Origin'] = '*'
                    logger.info(
                        f'{userID} [getIssueTicket] Response:Thank you, We have created Ticket for your concern.<br><b><center>Ticket ID&nbsp;<font size="5px" color="#174c82">"' + Ticket_id_ + '"</font></center>')

            print("Length of Ticket ID:", len(Ticket_id_))
            if len(Ticket_id_) < 20:
                if resolve.lower() == 'unanswered':
                    try:
                        getAnswered_query = "SELECT query_id FROM unanswered_queries where username='" + userName + "' order by asked_on desc  "
                        uns = create_query(getAnswered_query)
                        print(uns[0][0])
                        queryid = uns[0][0]
                        print("queryid " + str(queryid))
                    ...
                    try:
                        getAnswered_query = "SELECT query_id FROM answered_queries where username='" + userName + "' order by asked_on desc  "
                        uns = create_query(getAnswered_query)
                        print(uns[0][0])
                        queryid = uns[0][0]
                        print("queryid " + str(queryid))
...
def gettaglanguage(language):
    a={
        "gu":["માનવ સંસાધન","મારી ટિકિટની સ્થિતિ","મારા દસ્તાવેજો"],
        "hi": ["मानव संसाधन","मेरी टिकट की स्थिति","मेरे दस्तावेज"],
        "ml": ["മാനവ വിഭവശേഷി","എന്റെ ടിക്കറ്റ് നില","എന്റെ രേഖകള്"],
        "mr": ["मानव संसाधन","माझी तिकिट स्थिती","माझे कागदपत्र"],
        "ta": ["மனித வளம்","எனது டிக்கெட் நிலை","எனது ஆவணங்கள்"],
        "te": ["మానవ వనరుల","నా టికెట్ స్థితి","నా పత్రాలు"],
        "ur": ["انسانی وسائل","میری ٹکٹ کی حیثیت","میرے کاغذات"],
        "pa": ["ਮਾਨਵ ਸੰਸਾਧਨ","ਮੇਰੀ ਟਿਕਟ ਸਥਿਤੀ","ਮੇਰੇ ਦਸਤਾਵੇਜ਼"],
    }
    taglist = a.get(language)
    return taglist
...
def get_tag_list():
    ...
    try:
        ...
        for opData in output2:
            ...
            if locationId.lower() in valueLocList:
                locCount += 1
        print('loc count-->', locCount)
        notification_count = count + inviCount + locCount
        print("notify count---> ", notification_count)
        # query = "select * from notify_MyTaj  order by notification_id desc"
        # output = create_query(query)
        # tag_list = list(output)
